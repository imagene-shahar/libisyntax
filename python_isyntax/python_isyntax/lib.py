...
class LibISyntax:
    ...
    def __init__(self, library_path: str | None = None) -> None:
        if library_path is None:
            libname = {
                "darwin": "libisyntax.dylib",
                "win32": "libisyntax.dll",
            }.get(sys.platform, "libisyntax.so")
            env_path = os.environ.get("LIBISYNTAX_PATH")
            if env_path:
                library_path = env_path
            else:
                try:
                    library_path = str(resources.files(__package__).joinpath(libname))
                except Exception:
